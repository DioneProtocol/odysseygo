...
type State interface {
	...
	// ValidatorSet adds all the validators of [subnetID] into
	// [vdrs].
...
func new(
	...
	rewardUTXOsCache, err := metercacher.New[ids.ID, []*dione.UTXO](
		"reward_utxos_cache",
		metricsReg,
		&cache.LRU[ids.ID, []*dione.UTXO]{Size: rewardUTXOsCacheSize},
	)
	if err != nil {
		return nil, err
	}
	...
	utxoState, err := dione.NewMeteredUTXOState(utxoDB, txs.GenesisCodec, metricsReg)
	if err != nil {
		return nil, err
	}
...
func (s *state) init(genesisBytes []byte) error {
	...
	genesisBlock, err := blocks.NewOdysseyCommitBlock(genesisID, 0 /*height*/)
	if err != nil {
		return err
	}
