...
import (
	...
	"github.com/DioneProtocol/odysseygo/vms/omegavm/txs/txheap"
	"github.com/DioneProtocol/odysseygo/vms/secp256k1fx"
)

// Note that since an Odyssey network has exactly one Omega Chain,
// and the Omega Chain defines the genesis state of the network
// (who is staking, which chains exist, etc.), defining the genesis
// state of the Omega Chain is the same as defining the genesis
...
// StaticService defines the static API methods exposed by the omega VM
type StaticService struct{}

// UTXO is a UTXO on the Omega Chain that exists at the chain's genesis.
type UTXO struct {
...
// the genesis data of the Omega Chain.
// [NetworkID] is the ID of the network
// [UTXOs] are the UTXOs on the Omega Chain that exist at genesis.
// [Validators] are the validators of the primary network at genesis.
// [Chains] are the chains that exist at genesis.
// [Time] is the Omega Chain's time at network genesis.
...
// BuildGenesis build the genesis state of the Omega Chain (and thereby the Odyssey network.)
func (*StaticService) BuildGenesis(_ *http.Request, args *BuildGenesisArgs, reply *BuildGenesisReply) error {
	// Specify the UTXOs on the Omega chain that exist at genesis.
	...
	for i, apiUTXO := range args.UTXOs {
		...
		utxo := dione.UTXO{
			...
			Asset: dione.Asset{ID: args.DioneAssetID},
		...
		if apiUTXO.Locktime > args.Time {
			utxo.Out = &stakeable.LockOut{
				...
				TransferableOut: utxo.Out.(dione.TransferableOut),
			}
		}
