...
type Client interface {
	...
	// GetCurrentSupply returns an upper bound on the supply of DIONE in the system along with the O-chain height
	...
	// `odysseygo/wallet/chain/o.Wallet` utility.
	AddValidator(
		ctx context.Context,
		user api.UserPass,
		from []ids.ShortID,
...
func (c *client) AddDelegator(
	...
	res := &api.JSONTxID{}
	jsonStakeAmount := json.Uint64(stakeAmount)
	err := c.requester.SendRequest(ctx, "omega.addDelegator", &AddDelegatorArgs{
		JSONSpendHeader: api.JSONSpendHeader{
			UserPass:       user,
			JSONFromAddrs:  api.JSONFromAddrs{From: ids.ShortIDsToStrings(from)},
			JSONChangeAddr: api.JSONChangeAddr{ChangeAddr: changeAddr.String()},
...
func (c *client) GetBlockByHeight(ctx context.Context, height uint64, options ...rpc.Option) ([]byte, error) {
	res := &api.FormattedBlock{}
	err := c.requester.SendRequest(ctx, "omega.getBlockByHeight", &api.GetBlockByHeightArgs{
		Height:   json.Uint64(height),
