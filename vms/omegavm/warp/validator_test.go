...
func TestGetCanonicalValidatorSet(t *testing.T) {
	...
	tests := []test{
		...
		},
		{
			...
			stateF: func(ctrl *gomock.Controller) validators.State {
				state := validators.NewMockState(ctrl)
				state.EXPECT().GetValidatorSet(gomock.Any(), oChainHeight, subnetID).Return(
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			vdrs, weight, err := GetCanonicalValidatorSet(context.Background(), state, oChainHeight, subnetID)
...
func BenchmarkGetCanonicalValidatorSet(b *testing.B) {
	...
	for _, size := range []int{0, 1, 10, 100, 1_000, 10_000} {
		...
		b.Run(fmt.Sprintf("%d", size), func(b *testing.B) {
			for i := 0; i < b.N; i++ {
				_, _, err := GetCanonicalValidatorSet(context.Background(), validatorState, oChainHeight, subnetID)
				require.NoError(b, err)
			}
		})
	}
