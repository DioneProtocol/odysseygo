	...
	tests := []struct {
		...
		},
		{
			...
			stateF: func(ctrl *gomock.Controller) validators.State {
				...
				state.EXPECT().GetValidatorSet(gomock.Any(), oChainHeight, subnetID).Return(nil, errTest)
				return state
			},
			quorumNum: 1,
			quorumDen: 2,
		...
		},
		{
			...
			stateF: func(ctrl *gomock.Controller) validators.State {
				state := validators.NewMockState(ctrl)
				state.EXPECT().GetSubnetID(gomock.Any(), sourceChainID).Return(subnetID, nil)
				state.EXPECT().GetValidatorSet(gomock.Any(), oChainHeight, subnetID).Return(map[ids.NodeID]*validators.GetValidatorOutput{
					testVdrs[0].nodeID: {
						NodeID:    testVdrs[0].nodeID,
						PublicKey: testVdrs[0].vdr.PublicKey,
		...
		},
		{
			...
			stateF: func(ctrl *gomock.Controller) validators.State {
				state := validators.NewMockState(ctrl)
				state.EXPECT().GetSubnetID(gomock.Any(), sourceChainID).Return(subnetID, nil)
				state.EXPECT().GetValidatorSet(gomock.Any(), oChainHeight, subnetID).Return(vdrs, nil)
				return state
			},
			quorumNum: 1,
			quorumDen: 2,
		...
		},
		{
			...
			stateF: func(ctrl *gomock.Controller) validators.State {
				state := validators.NewMockState(ctrl)
				state.EXPECT().GetSubnetID(gomock.Any(), sourceChainID).Return(subnetID, nil)
				state.EXPECT().GetValidatorSet(gomock.Any(), oChainHeight, subnetID).Return(nil, nil)
				return state
			},
			quorumNum: 1,
			quorumDen: 2,
