...
func TestGetValidatorRules(t *testing.T) {
	...
	var (
		config = &config.Config{
			...
			MaxValidatorStakeDuration:       4 * time.Second,
			PyruniMaxValidatorStakeDuration: 3 * time.Second,
			MinDelegatorStakeDuration:       2 * time.Second,
			PyruniMinDelegatorStakeDuration: time.Second,
			MaxDelegatorStakeDuration:       4 * time.Second,
			PyruniMaxDelegatorStakeDuration: 3 * time.Second,
	...
	tests := []test{
		...
		},
		{
			...
			expectedRules: &addValidatorRules{
				...
				maxStakeDuration:  config.PyruniMaxValidatorStakeDuration,
				minDelegationFee:  config.MinDelegationFee,
			},
		},
		{
	...
	tests := []test{
		...
		},
		{
			...
			expectedRules: &addDelegatorRules{
				...
				maxStakeDuration:         config.PyruniMaxDelegatorStakeDuration,
				maxValidatorWeightFactor: MaxValidatorWeightFactor,
			},
		},
		{
