...
func TestTransformSubnetTxSerialization(t *testing.T) {
	...
	simpleTransformTx := &TransformSubnetTx{
		...
		MaxValidatorStakeDuration: 365 * 24 * 60 * 60,
		MinDelegatorStakeDuration: 1,
		MaxDelegatorStakeDuration: 365 * 24 * 60 * 60,
	...
	require.NoError(simpleTransformTx.SyntacticVerify(&snow.Context{
		NetworkID:    1,
		ChainID:      constants.OmegaChainID,
	...
	expectedUnsignedSimpleTransformTxBytes := []byte{
		...
		0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
		// minimum validator staking duration
		0x00, 0x00, 0x00, 0x01,
		// maximum validator staking duration
		0x01, 0xe1, 0x33, 0x80,
		// minimum delegator staking duration
		0x00, 0x00, 0x00, 0x01,
		// maximum delegator staking duration
		0x01, 0xe1, 0x33, 0x80,
	...
	complexTransformTx := &TransformSubnetTx{
		...
		MinValidatorStakeDuration: 1,
		MaxValidatorStakeDuration: 1,
		MinDelegatorStakeDuration: 1,
		MaxDelegatorStakeDuration: 1,
		MinDelegationFee:          0,
...
func TestTransformSubnetTxSyntacticVerify(t *testing.T) {
	...
	tests := []test{
		...
		{
			name: "minValidatorStakeDuration == 0",
			txFunc: func(*gomock.Controller) *TransformSubnetTx {
				return &TransformSubnetTx{
					BaseTx:                    validBaseTx,
					Subnet:                    ids.GenerateTestID(),
		...
		{
			name: "minDelegatorStakeDuration == 0",
			txFunc: func(*gomock.Controller) *TransformSubnetTx {
				return &TransformSubnetTx{
					BaseTx:                    validBaseTx,
					Subnet:                    ids.GenerateTestID(),
					...
					MinDelegatorStakeDuration: 0,
				}
			},
			err: errMinStakeDurationZero,
		},
		{
			name: "minValidatorStakeDuration > maxValidatorStakeDuration",
			txFunc: func(*gomock.Controller) *TransformSubnetTx {
				return &TransformSubnetTx{
					BaseTx:                    validBaseTx,
					Subnet:                    ids.GenerateTestID(),
		...
		{
			name: "minDelegatorStakeDuration > maxDelegatorStakeDuration",
			txFunc: func(*gomock.Controller) *TransformSubnetTx {
				return &TransformSubnetTx{
					BaseTx:                    validBaseTx,
					Subnet:                    ids.GenerateTestID(),
