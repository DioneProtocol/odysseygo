...
func defaultContext() *snow.Context {
	...
	errs.Add(
		...
		aliaser.Alias(aChainID, "A"),
		aliaser.Alias(aChainID, aChainID.String()),
		aliaser.Alias(dChainID, "C"),
		aliaser.Alias(dChainID, dChainID.String()),
	)
	if errs.Errored() {
		panic(errs.Err)
	}
...
func TestOptimisticAtomicImport(t *testing.T) {
	...
	statelessBlk, err := blocks.NewOdysseyAtomicBlock(
		preferredID,
		preferredHeight+1,
		tx,
	)
