...
func NewContext(tb testing.TB) *snow.Context {
	...
	errs := wrappers.Errs{}
	errs.Add(
		aliaser.Alias(chainID, "A"),
		aliaser.Alias(chainID, chainID.String()),
		aliaser.Alias(constants.OmegaChainID, "O"),
		aliaser.Alias(constants.OmegaChainID, constants.OmegaChainID.String()),
	)
	if errs.Errored() {
		tb.Fatal(errs.Err)
	}
...
func GenesisVMWithArgs(tb testing.TB, additionalFxs []*common.Fx, args *BuildGenesisArgs) ([]byte, chan common.Message, *VM, *atomic.Memory) {
	...
	if err != nil {
		tb.Fatal("should not have caused error in creating alpha config bytes")
	}
	err = vm.Initialize(
		context.Background(),
		ctx,
...
func setupIssueTx(t testing.TB) (chan common.Message, *VM, *snow.Context, []*txs.Tx) {
	...
	return issuer, vm, ctx, []*txs.Tx{dioneTx, firstTx, secondTx}
}
		...
		BaseTx: dione.BaseTx{
			...
			Ins: []*dione.TransferableInput{
				// fee asset
				{
					...
					Asset: dione.Asset{ID: feeAssetCreateTx.ID()},
					In: &secp256k1fx.TransferInput{
						Amt: startBalance,
						Input: secp256k1fx.Input{
							SigIndices: []uint32{
...
func TestVMFormat(t *testing.T) {
	...
	tests := []struct {
		...
		{ids.ShortEmpty, "A-testing1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqtu2yas"},
	}
...
func TestIssueImportTx(t *testing.T) {
	...
	alphaConfigBytes, err := stdjson.Marshal(alphaConfig)
	require.NoError(t, err)
	vm := &VM{}
	err = vm.Initialize(
		context.Background(),
	...
	assertLatestIdx(t, vm.db, key.PublicKey().Address(), dioneID, 1)
