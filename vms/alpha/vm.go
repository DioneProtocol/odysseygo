...
import (
	...
	blockbuilder "github.com/DioneProtocol/odysseygo/vms/alpha/block/builder"
	blockexecutor "github.com/DioneProtocol/odysseygo/vms/alpha/block/executor"
	extensions "github.com/DioneProtocol/odysseygo/vms/alpha/fxs"
	txexecutor "github.com/DioneProtocol/odysseygo/vms/alpha/txs/executor"
)
...
func (vm *VM) Initialize(
	...
	if len(configBytes) > 0 {
		if err := stdjson.Unmarshal(configBytes, &alphaConfig); err != nil {
			return err
		}
		ctx.Log.Info("VM config initialized",
			zap.Reflect("config", alphaConfig),
		)
	}
	...
	if alphaConfig.IndexTransactions {
		vm.ctx.Log.Warn("deprecated address transaction indexing is enabled")
		vm.addressTxsIndexer, err = index.NewIndexer(vm.db, vm.ctx.Log, "", vm.registerer, alphaConfig.IndexAllowIncomplete)
		if err != nil {
		...
		vm.addressTxsIndexer, err = index.NewNoIndexer(vm.db, alphaConfig.IndexAllowIncomplete)
		if err != nil {
...
func (vm *VM) CreateHandlers(context.Context) (map[string]*common.HTTPHandler, error) {
	...
	// name this service "alpha"
	if err := rpcServer.RegisterService(&Service{vm: vm}, "alpha"); err != nil {
		return nil, err
	}
...
func (*VM) CreateStaticHandlers(context.Context) (map[string]*common.HTTPHandler, error) {
	...
	}, newServer.RegisterService(staticService, "alpha")
}
