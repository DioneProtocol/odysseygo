...
func TestInitialStateVerifyInvalidOutput(t *testing.T) {
	c := linearcodec.NewDefault()
	if err := c.RegisterType(&dione.TestVerifiable{}); err != nil {
		t.Fatal(err)
	}
	m := codec.NewDefaultManager()
	if err := m.RegisterCodec(CodecVersion, c); err != nil {
	...
	is := InitialState{
		FxIndex: 0,
		Outs:    []verify.State{&dione.TestVerifiable{Err: errTest}},
	}
	if err := is.Verify(m, numFxs); err == nil {
...
func TestInitialStateVerifyUnsortedOutputs(t *testing.T) {
	c := linearcodec.NewDefault()
	if err := c.RegisterType(&dione.TestTransferable{}); err != nil {
		t.Fatal(err)
	}
	m := codec.NewDefaultManager()
	if err := m.RegisterCodec(CodecVersion, c); err != nil {
