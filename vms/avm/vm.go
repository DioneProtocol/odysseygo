...
import (
	...
	extensions "github.com/dioneprotocol/dionego/vms/avm/fxs"
)
...
func (vm *VM) Initialize(
	...
	vm.AtomicUTXOManager = dione.NewAtomicUTXOManager(ctx.SharedMemory, vm.parser.Codec())
...
func (vm *VM) getUTXO(utxoID *dione.UTXOID) (*dione.UTXO, error) {
...
func (vm *VM) verifyTransferOfUTXO(utx txs.UnsignedTx, in *dione.TransferableInput, cred verify.Verifiable, utxo *dione.UTXO) error {
...
func (vm *VM) verifyTransfer(tx txs.UnsignedTx, in *dione.TransferableInput, cred verify.Verifiable) error {
...
func (vm *VM) Spend(
	...
	dione.SortTransferableInputsWithSigners(ins, keys)
...
func (vm *VM) SpendNFT(
	...
	for _, utxo := range utxos {
		...
		ops = append(ops, &txs.Operation{
			Asset:   utxo.Asset,
			UTXOIDs: []*dione.UTXOID{&utxo.UTXOID},
