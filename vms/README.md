...
There is a special subnet inherent to the Odyssey Network called the Primary Network. The Primary Network is validated by every node on the Odyssey network. All subnets' validator sets are required to be a subset of the Primary Network's validator set. That is, if a validator belongs to a subnet then it also belongs to the Primary Network. The Primary Network validates three blockchains that are inherent to the Odyssey Network: the O-Chain, D-Chain, and X-Chain.
