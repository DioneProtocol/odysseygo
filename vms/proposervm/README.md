...
- `OChainHeight` the height of the last accepted block on the O-chain at the time the block is produced.
...
For a given block, Snowman++ randomly selects a list of proposers. Block proposers are selected from the subnet's validators. Snowman++ extracts the list of a given subnet's validators from the O-Chain. Let a block have a height `H` and O-Chain height `O` recorded in its header. The proposers list for next block is generated independently but reproducibly by each node as follows:

- Subnet validators active at block `O` are retrieved from O-chain.
...
- Given a `proposervm.Block` **C** and its parent block **O**, **O**'s inner block must be **C**'s inner block's parent.
- A block must have a `OChainHeight` is larger or equal to its parent's `OChainHeight` (`OChainHeight` is monotonic).
- A block must have a `OChainHeight` that is less or equal to current O-Chain height.
