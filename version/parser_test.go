...
func TestParseApplication(t *testing.T) {
	v, err := ParseApplication("odyssey/1.2.3")

	require.NoError(t, err)
	require.NotNil(t, v)
	require.Equal(t, "odyssey/1.2.3", v.String())
	require.Equal(t, 1, v.Major)
	require.Equal(t, 2, v.Minor)
	require.Equal(t, 3, v.Patch)
	...
	tests := []struct {
		...
		{
			version:     "odyssey/z.0.0",
			expectedErr: strconv.ErrSyntax,
		},
		{
			version:     "odyssey/0.z.0",
			expectedErr: strconv.ErrSyntax,
		},
		{
			version:     "odyssey/0.0.z",
			expectedErr: strconv.ErrSyntax,
		},
		{
			version:     "odyssey/0.0.0.0",
			expectedErr: strconv.ErrSyntax,
		},
	}
	for _, test := range tests {
