...
**These API format changes are breaking changes. https://api.dione.network and https://api.dione-test.network have been updated with this format. If you are using OdysseyGo APIs in your code, please ensure you have updated to the latest versions. See  https://docs.dione.network/apis/odysseygo/cb58-deprecation for details about the CB58 removal.**
...
- Suppressed the super cool `odysseygo` banner when `stdout` is not directed to a terminal
- Updated linter version
...
- Added `odyssey_{chainID}_handler_async_expired` metric
...
- Replaced OdysseyGo's internal logger with [uber-go/zap](https://github.com/uber-go/zap).
- Replaced OdysseyGo's log rotation with [lumberjack](https://github.com/natefinch/lumberjack).
...
- Added CPU usage metrics for OdysseyGo and all sub-processes
- Added Disk IO usage metrics for OdysseyGo and all sub-processes
...
- Added OdysseyGo custom log formatting to the logs.
...
- Added support for arbitrarily deleted log files while odysseygo is running.
...
- Fixed error reporting when making Odyssey chains that did not manually specify a primary alias.
...
- Introduced a new vertex type to support future `Odyssey` based network upgrades.
...
- Added `--stake-supply-cap` which defaults to `720,000,000,000,000,000` nDIONE.
...
- Fixed 32-bit architecture builds for OdysseyGo (not Coreth).
...
- Added `odyssey_{ChainID}_bootstrap_finished`, which is 1 if the chain is done bootstrapping, 0 otherwise.
...
- Updated `odyssey_{ChainID}_handler_gossip_{count,sum}` to `odyssey_{ChainID}_handler_gossip_request_{count,sum}`.
- Updated `odyssey_{ChainID}_lat_get_accepted_{count,sum}` to `odyssey_{ChainID}_lat_accepted_{count,sum}`.
- Updated `odyssey_{ChainID}_lat_get_accepted_frontier_{count,sum}` to `odyssey_{ChainID}_lat_accepted_frontier_{count,sum}`.
- Updated `odyssey_{ChainID}_lat_get_ancestors_{count,sum}` to `odyssey_{ChainID}_lat_multi_put_{count,sum}`.
- Combined `odyssey_{ChainID}_lat_pull_query_{count,sum}` and `odyssey_{ChainID}_lat_push_query_{count,sum}` to `odyssey_{ChainID}_lat_chits_{count,sum}`.
- Added `odyssey_{ChainID}_app_response_{count,sum}`.
- Added `odyssey_network_bandwidth_throttler_inbound_acquire_latency_{count,sum}`
- Added `odyssey_network_bandwidth_throttler_inbound_awaiting_acquire`
- Added `odyssey_P_vm_votes_won`
- Added `odyssey_P_vm_votes_lost`
...
- Changed the behavior of `odyssey_network_buffer_throttler_inbound_awaiting_acquire` to only increment if the message is actually blocking.
- Changed the behavior of `odyssey_network_byte_throttler_inbound_awaiting_acquire` to only increment if the message is actually blocking.
- Added `Block/Tx` metrics on `meterVM`s.
  - Added `odyssey_{ChainID}_vm_metervm_build_block_err_{count,sum}`.
  - Added `odyssey_{ChainID}_vm_metervm_parse_block_err_{count,sum}`.
  - Added `odyssey_{ChainID}_vm_metervm_get_block_err_{count,sum}`.
  - Added `odyssey_{ChainID}_vm_metervm_verify_{count,sum}`.
  - Added `odyssey_{ChainID}_vm_metervm_verify_err_{count,sum}`.
  - Added `odyssey_{ChainID}_vm_metervm_accept_{count,sum}`.
  - Added `odyssey_{ChainID}_vm_metervm_reject_{count,sum}`.
  - Added `odyssey_{DAGID}_vm_metervm_parse_tx_err_{count,sum}`.
  - Added `odyssey_{DAGID}_vm_metervm_get_tx_err_{count,sum}`.
  - Added `odyssey_{DAGID}_vm_metervm_verify_tx_{count,sum}`.
  - Added `odyssey_{DAGID}_vm_metervm_verify_tx_err_{count,sum}`.
  - Added `odyssey_{DAGID}_vm_metervm_accept_{count,sum}`.
  - Added `odyssey_{DAGID}_vm_metervm_reject_{count,sum}`.
