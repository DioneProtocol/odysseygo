...
The Odyssey primary network consists of 3 built-in blockchains: the A-Chain, D-Chain, and O-Chain. All three chains rely on the Snowman consensus protocol. The A-Chain, which previously used DAG-based Odyssey consensus, was upgraded to Snowman in the Cortina network update. 

The A-Chain is used to manage assets. The D-Chain is used to create and interact with smart contracts. The O-Chain is used to coordinate validators and stake. At the time of writing, the Odyssey network has ~1200 validators. A set of validators makes up a subnet. Subnets can validate 1 or more chains. It is a common misconception that 1 subnet = 1 chain and this is shown by the primary subnet of Odyssey which is made up of the A-Chain, D-Chain, and O-Chain.
...
Each blockchain on Odyssey has several components: the virtual machine, database, consensus engine, sender, and handler. These components help the chain run smoothly. Blockchains also interact with the P2P layer and the chain router to send and receive messages.

## P2P

### [Outbound Messages](https://github.com/DioneProtocol/odysseygo/blob/master/message/outbound_msg_builder.go)
...
Currently, Odysseygo implements its own message serialization to communicate. In the future, Odysseygo will use protocol buffers to communicate.

### [Network](https://github.com/DioneProtocol/odysseygo/blob/master/network/network.go)
...
## [Sender](https://github.com/DioneProtocol/odysseygo/blob/master/snow/networking/sender/sender.go)
...
## [Consensus Engine](https://github.com/DioneProtocol/odysseygo/blob/master/snow/consensus/snowman/consensus.go)

Consensus is defined as getting a group of distributed systems to agree on an outcome. In the case of the Odyssey network, consensus is achieved when validators are in agreement with the state of the blockchain. The novel consensus algorithm is documented in the [white paper](https://assets.website-files.com/5d80307810123f5ffbb34d6e/6009805681b416f34dcae012_Odyssey%20Consensus%20Whitepaper.pdf). There are two main consensus algorithms: Odyssey and Snowman. The engine is responsible for proposing a new block to consensus, repeatedly polling the network for decisions (accept/reject), and communicating that decision to the `Sender`.
