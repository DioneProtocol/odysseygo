...
import (
	...
	"github.com/dioneprotocol/dionego/snow/consensus/metrics"
...
// Topological performs the dione algorithm by utilizing a topological sort
...
func (ta *Topological) HealthCheck(ctx context.Context) (interface{}, error) {
	...
	if !healthy {
		...
		return details, fmt.Errorf("dione consensus is not healthy reason: %s", strings.Join(errorReasons, ", "))
	}
	return details, nil
}
