...
import (
	...
	"github.com/dioneprotocol/dionego/utils/compare"
...
func AddTest(t *testing.T, factory Factory) {
	...
	if !avl.Finalized() {
		t.Fatal("An empty dione instance is not finalized")
	}
...
func VotingTest(t *testing.T, factory Factory) {
	...
	case err != nil:
		t.Fatal(err)
	case avl.Finalized():
		t.Fatalf("An dione instance finalized too early")
	...
	case err != nil:
		t.Fatal(err)
	case !avl.Finalized():
		t.Fatalf("An dione instance finalized too late")
	case !compare.UnsortedEquals([]ids.ID{vtx1.IDV}, avl.Preferences().List()):
...
func SplitVotingTest(t *testing.T, factory Factory) {
	...
	case err != nil:
		t.Fatal(err)
	case avl.Finalized(): // dione shouldn't be finalized because the vertex transactions are still processing
		t.Fatalf("An dione instance finalized too late")
...
func TransactionVertexTest(t *testing.T, factory Factory) {
	...
	case !avl.Finalized():
		t.Fatal("expected finalized dione instance")
