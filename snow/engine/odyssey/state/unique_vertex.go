...
var (
	...
	_ odyssey.Vertex        = (*uniqueVertex)(nil)
)
...
func (vtx *uniqueVertex) Parents() ([]odyssey.Vertex, error) {
	vtx.refresh()
	...
	if len(vtx.v.parents) != len(parentIDs) {
		vtx.v.parents = make([]odyssey.Vertex, len(parentIDs))
