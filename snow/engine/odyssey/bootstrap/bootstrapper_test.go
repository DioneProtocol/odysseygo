...
import (
	...
	"github.com/DioneProtocol/odysseygo/snow/engine/odyssey/getter"
...
func newConfig(t *testing.T) (Config, ids.NodeID, *common.SenderTest, *vertex.TestManager, *vertex.TestVM) {
	...
	vtxBlocker, err := queue.NewWithMissing(prefixdb.New([]byte("vtx"), db), "vtx", ctx.OdysseyRegisterer)
	require.NoError(err)

	txBlocker, err := queue.New(prefixdb.New([]byte("tx"), db), "tx", ctx.OdysseyRegisterer)
	require.NoError(err)
	...
	dioneGetHandler, err := getter.New(manager, commonConfig)
	require.NoError(err)
...
func TestBootstrapperSingleFrontier(t *testing.T) {
	...
	vtx2 := &odyssey.TestVertex{ // vtx2 is the stop vertex
		TestDecidable: choices.TestDecidable{
			IDV:     vtxID2,
			StatusV: choices.Processing,
		},
	...
	manager.ParseVtxF = func(_ context.Context, vtxBytes []byte) (odyssey.Vertex, error) {
		switch {
...
func TestBootstrapperByzantineResponses(t *testing.T) {
	...
	vtx1 := &odyssey.TestVertex{ // vtx1 is the stop vertex
		TestDecidable: choices.TestDecidable{
			IDV:     vtxID1,
			StatusV: choices.Processing,
		},
		ParentsV: []odyssey.Vertex{vtx0},
		HeightV:  1,
		BytesV:   vtxBytes1,
	}
	// Should not receive transitive votes from [vtx1]
	vtx2 := &odyssey.TestVertex{
		TestDecidable: choices.TestDecidable{
			IDV:     vtxID2,
			StatusV: choices.Unknown,
		},
...
func TestBootstrapperTxDependencies(t *testing.T) {
	...
	vtx1 := &odyssey.TestVertex{ // vtx1 is the stop vertex
		...
		ParentsV: []odyssey.Vertex{vtx0}, // Depends on vtx0
		HeightV:  1,
...
func TestRestartBootstrapping(t *testing.T) {
	...
	vtx3 := &odyssey.TestVertex{
		TestDecidable: choices.TestDecidable{
			IDV:     vtxID3,
			StatusV: choices.Unknown,
		},
		ParentsV: []odyssey.Vertex{vtx2},
		HeightV:  3,
		BytesV:   vtxBytes3,
	}
	vtx4 := &odyssey.TestVertex{
		TestDecidable: choices.TestDecidable{
			IDV:     vtxID4,
			StatusV: choices.Unknown,
		},
	...
	vtx5 := &odyssey.TestVertex{ // vtx5 is the stop vertex
		TestDecidable: choices.TestDecidable{
			IDV:     vtxID5,
			StatusV: choices.Unknown,
		},
		ParentsV: []odyssey.Vertex{vtx3, vtx4},
		HeightV:  4,
		BytesV:   vtxBytes5,
	}
