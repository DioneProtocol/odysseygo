...
import (
	...
	"github.com/dioneprotocol/dionego/snow/consensus/dione/poll"
...
func (t *Transitive) Start(ctx context.Context, startReqID uint32) error {
	...
	edge := t.Manager.Edge(ctx)
	frontier := make([]dione.Vertex, 0, len(edge))
...
func (t *Transitive) GetVtx(ctx context.Context, vtxID ids.ID) (dione.Vertex, error) {
...
// Returns true if [vtx] has been added to consensus (now or previously)
func (t *Transitive) issueFrom(ctx context.Context, nodeID ids.NodeID, vtx dione.Vertex) (bool, error) {
	issued := true
...
// Assumes we have [vtx].
func (t *Transitive) issue(ctx context.Context, vtx dione.Vertex) error {
	vtxID := vtx.ID()
