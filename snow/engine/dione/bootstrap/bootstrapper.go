...
func (b *bootstrapper) Ancestors(ctx context.Context, nodeID ids.NodeID, requestID uint32, vtxs [][]byte) error {
	...
	processVertices := make([]dione.Vertex, 1, len(vtxs)) // Process all of the valid vertices in this message
...
func (b *bootstrapper) process(ctx context.Context, vtxs ...dione.Vertex) error {
...
func (b *bootstrapper) ForceAccepted(ctx context.Context, acceptedContainerIDs []ids.ID) error {
	...
	toProcess := make([]dione.Vertex, 0, len(pendingContainerIDs))
