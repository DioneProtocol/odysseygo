...
import (
	...
	"github.com/dioneprotocol/dionego/snow/engine/dione/getter"
...
func TestBootstrapperSingleFrontier(t *testing.T) {
	...
	bs, err := New(
		...
		func(context.Context, uint32) error {
			config.Ctx.State.Set(snow.EngineState{
				Type:  p2p.EngineType_ENGINE_TYPE_DIONE,
				State: snow.NormalOp,
			})
			return nil
		},
	...
	manager.ParseVtxF = func(_ context.Context, vtxBytes []byte) (dione.Vertex, error) {
		switch {
...
func TestBootstrapperTxDependencies(t *testing.T) {
	...
	vtx1 := &dione.TestVertex{
		...
		ParentsV: []dione.Vertex{vtx0}, // Depends on vtx0
		HeightV:  1,
...
func TestRestartBootstrapping(t *testing.T) {
	...
	vtx3 := &dione.TestVertex{
		TestDecidable: choices.TestDecidable{
			IDV:     vtxID3,
			StatusV: choices.Unknown,
		},
		ParentsV: []dione.Vertex{vtx2},
		HeightV:  3,
		BytesV:   vtxBytes3,
	}
	vtx4 := &dione.TestVertex{
		TestDecidable: choices.TestDecidable{
			IDV:     vtxID4,
			StatusV: choices.Unknown,
		},
	...
	vtx5 := &dione.TestVertex{
		TestDecidable: choices.TestDecidable{
			IDV:     vtxID5,
			StatusV: choices.Unknown,
		},
		ParentsV: []dione.Vertex{vtx4},
		HeightV:  4,
		BytesV:   vtxBytes5,
	}
