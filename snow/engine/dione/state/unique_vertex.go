...
var (
	...
	_ dione.Vertex        = (*uniqueVertex)(nil)
)
...
func (vtx *uniqueVertex) Parents() ([]dione.Vertex, error) {
	vtx.refresh()
	...
	if len(vtx.v.parents) != len(parentIDs) {
		vtx.v.parents = make([]dione.Vertex, len(parentIDs))
