...
const (
	metricVtxIssueSuccess = "dione_X_whitelist_vtx_issue_success"
	metricVtxIssueFailure = "dione_X_whitelist_vtx_issue_failure"
	metricTxProcessing    = "dione_X_whitelist_tx_processing"
	metricTxAccepted      = "dione_X_whitelist_tx_accepted_count"
	metricTxRejected      = "dione_X_whitelist_tx_rejected_count"
	metricTxPollsAccepted = "dione_X_whitelist_tx_polls_accepted_count"
	metricTxPollsRejected = "dione_X_whitelist_tx_polls_rejected_count"
)
	...
	ginkgo.It("can issue whitelist vtx",
		...
		func() {
			...
			ginkgo.By("issue regular, virtuous X-Chain tx, before whitelist vtx, should succeed", func() {
				...
				balances, err = wallets[0].X().Builder().GetFTBalance()
				gomega.Expect(err).Should(gomega.BeNil())
				key1CurBalX := balances[dioneAssetID]
				tests.Outf("{{green}}first wallet balance:{{/}} %d\n", key1CurBalX)

				balances, err = wallets[1].X().Builder().GetFTBalance()
				gomega.Expect(err).Should(gomega.BeNil())
				key2CurBalX := balances[dioneAssetID]
				tests.Outf("{{green}}second wallet balance:{{/}} %d\n", key2CurBalX)
