...
if [[ ${GOOS} == "darwin" ]]; then
  DOWNLOAD_URL="https://github.com/DioneProtocol/odyssey-network-runner/releases/download/v${NETWORK_RUNNER_VERSION}/odyssey-network-runner_${NETWORK_RUNNER_VERSION}_darwin_amd64.tar.gz"
fi

rm -f ${DOWNLOAD_PATH}
rm -f /tmp/odyssey-network-runner

echo "downloading odyssey-network-runner ${NETWORK_RUNNER_VERSION} at ${DOWNLOAD_URL}"
curl -L ${DOWNLOAD_URL} -o ${DOWNLOAD_PATH}
