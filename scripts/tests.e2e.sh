...
# e.g.,
# ./scripts/build.sh
# ./scripts/tests.e2e.sh ./build/dionego
# ENABLE_WHITELIST_VTX_TESTS=true ./scripts/tests.e2e.sh ./build/dionego
...
DIONEGO_PATH="${1-}"
if [[ -z "${DIONEGO_PATH}" ]]; then
  echo "Missing DIONEGO_PATH argument!"
  echo "Usage: ${0} [DIONEGO_PATH]" >> /dev/stderr
  exit 255
fi
...
#################################
echo "running e2e tests against the local cluster with ${DIONEGO_PATH}"
./tests/e2e/e2e.test \
--ginkgo.v \
--log-level debug \
--network-runner-grpc-endpoint="0.0.0.0:12342" \
--network-runner-dionego-path=${DIONEGO_PATH} \
--network-runner-dionego-log-level="WARN" \
